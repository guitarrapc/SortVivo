...
| Layer 2 | Quick Access | ワンタップで開く操作パネル | サイドバー / オーバーレイ |
...
| 要素 | 説明 |
|------|------|
| ヘッダー | アプリ名（中央）、☰（Quick Access 開閉）、🎓（Tutorial）、⚙（Settings） |
| 再生コントロール | ▶/⏸, ⏹, 🔊/🔇（可視化エリアの上） |
| Sort Card | 可視化 + Stats Summary |
| シークバー | N=1 時のみ |
| ステータス | N>1 時の "X/N completed"（再生コントロール横） |

**ヘッダーボタン仕様:**

| ボタン | 動作（PC） | 動作（タブレット/スマホ） |
|--------|-----------|----------------------|
| ☰ | Quick Access パネルの折りたたみ/展開トグル | Quick Access パネルのオーバーレイ表示 |
| 🎓 | `/tutorial/{currentAlgorithm}` に遷移 | 同左 |
| ⚙ | Settings モーダルを開く | 同左 |
...
| 🎯 Sort Target | Algorithm（ドロップダウン + ? リンク）、Array Size、Scramble Pattern |
| 🎨 Display | Visualization Mode |
| ⚡ Speed | Operations Per Frame（スライダー）、Speed Multiplier（スライダー + FPS） |
| 🚀 Action | Add & Generate（Upsert）、Complexity（読み取り専用） |

### 2.3 Layer 3: Settings（設定モーダル）

| 要素 | デフォルト |
|------|-----------|
| Renderer（Canvas 2D / WebGL） | Canvas 2D |
...
|----------|----|-------------|----------------|
| PC | ≥1280px | 左サイドバー（常時表示） | メインエリア |
| タブレット | 768〜1279px | 左オーバーレイ（☰で開閉） | フル幅 |
| スマホ | 〜767px | 左オーバーレイ（☰で開閉） | フル幅 |
...
N>1 時: Sort Card をグリッド表示（シークバーなし、"X/N completed" 表示）
...
N>1: **カルーセル方式**（横スワイプで1枚ずつ、ドットインジケーター + 📊 ボタン）
...
| デバイス | 表示方式 | 折りたたみ/閉じる |
|----------|---------|----------------|
| PC | 左サイドバー（折りたたみ可、幅 200〜500px、localStorage に保存） | ヘッダー ☰ またはパネル内 ☰ |
| タブレット | 左オーバーレイ | ヘッダー ☰ |
| スマホ | 左オーバーレイ | ヘッダー ☰ |
...
|------|-----------|--------------|---------|--------|
| アルゴリズム選択 | QAP: select + ? リンク | toolbar: select | 🎯 Sort Target | △ UI 同形、バインド先が異なる |
| Array Size / Pattern | QAP: select | ❌ | 🎯 Sort Target | ✕ メイン専用 |
| Visualization Mode | QAP: select | ❌ | 🎨 Display | ✕ メイン専用 |
| Ops/Frame + Multiplier | QAP: slider × 2 | ❌ | ⚡ Speed | ✕ メイン専用 |
| Speed (ms) | ❌ | toolbar: slider | ⚡ Speed | ✕ Tutorial 専用 |
| Play/Pause | PlayControlBar | toolbar | ▶ Playback | ✅ 概念共通 |
| Stop / Sound | PlayControlBar | ❌ | ▶ Playback | ✕ メイン専用 |
| Step 操作 / カウンター | ❌ | toolbar | ▶ Playback | ✕ Tutorial 専用 |
| Add & Generate | QAP | ❌ | 🚀 Action | ✕ メイン専用 |
| Complexity | QAP / SortCard badge | 説明パネル badge | 🚀 Action | △ 情報同一、表示形式が異なる |
...
- **メイン → チュートリアル**: QAP の Algorithm 選択横 `?` アイコン → `/tutorial/{algo}`
- **チュートリアル → メイン**: "▶ Try this algorithm" ボタン → メイン画面でそのアルゴリズムを選択済みで遷移
...
Index.razor
├── Header（タイトル + 🎓 + ⚙）
├── QuickAccessPanel（PC: 左サイドバー / タブレット・スマホ: 左オーバーレイ）
│   ├── AlgorithmDescriptionCard
│   ├── 🎯 SortTargetCard
│   ├── 🎨 DisplayCard
│   ├── ⚡ SpeedCard
│   └── 🚀 ActionCard
├── PlayControlBar（▶/⏸, ⏹, 🔊/🔇, ステータス）
├── SortCardArea
...
TutorialPage.razor
├── Header（← Back + タイトル + ⚙）
├── TutorialPlayControlBar（Step Counter + ◀◀ ◀ ▶/⏸ ▶ ▶▶ + Keyboard Hint）
├── DescriptionPanel
├── NarrativePanel
...
| コンポーネント | 状態 | 説明 |
|--------------|------|------|
| `SortCard.razor` | **新規** | CardHeader + Visualization + SortStatsSummary |
| `SortCardGrid.razor` | **新規** | 比較モードのグリッドコンテナ |
| `SortStatsSummary.razor` | **リネーム** | `ComparisonStatsSummary` → `SortStatsSummary`（タップ展開追加） |
| `PlayControlBar.razor` | **新規** | ▶/⏸, ⏹, 🔊/🔇 + ステータス |
| `QuickAccessPanel.razor` | **新規** | Layer 2 コンテナ（UI Card 縦積み） |
| `SettingsModal.razor` | **新規** | Layer 3 設定モーダル |
| `StatisticsPanel.razor` | **廃止** | SortStatsSummary に統合 |
| `NormalModeControls.razor` | **廃止** | 各コンポーネントに分散 |
...
| `--bg-panel` | `#1c2320` | Quick Access Panel |
...
| `--color-button-gradient-start` | `#5a9a4d` | ボタングラデーション開始 |
| `--color-button-gradient-end` | `#3d7a38` | ボタングラデーション終了 |
