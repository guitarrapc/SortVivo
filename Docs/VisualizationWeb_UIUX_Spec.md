...
| 条件 | 表示 |
|------|------|
| N≤1 | Sort Card の下に表示（N=0 の空状態でも表示） |
| N>1 | 非表示 |
...
- **メイン → チュートリアル**: QAP の Algorithm 選択横 `?` リンク → `/tutorial/{algo}`、またはヘッダーの `?` アイコン → `/tutorial`
