...
|--------|------|---------|--------|---------|
| Play/Pause | 停止/一時停止中 | `▶` | `var(--color-play)` = `#3b82f6`（青） | "Play" |
| Play/Pause | 再生中 | `⏸` | `var(--color-pause)` = `#a855f7`（紫） | "Pause" |
| Stop | 常時 | `⏹` | `var(--color-stop)` = `#e53e3e`（赤） | "Stop" |
| Sound | ON | `🔊` | `var(--color-sound-on)` = `#22c55e`（鮮やかな緑） | "Sound ON" |
| Sound | OFF | `🔇` | `var(--color-sound-off)` = `#6b7280`（グレー）+ `opacity: 0.6` | "Sound OFF" |
...
| 要素 | スタイル |
|------|---------|
| カード背景 | `var(--bg-card)` = `#272f3b`（パネル背景 `--bg-panel` より明るい） |
| カード見出し | アイコン + ラベル、`font-size: 0.72rem`、`text-transform: uppercase` |
| カード間マージン | `8px` |
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│ Try this algorithm  🎓 Tutorial: BubbleSort                                 │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
TutorialPage.razor
├── Header（Try this algorithm + Tutorialタイトル）
├── TutorialToolbar
│   ├── 🎯 SortTargetCard    ← Algorithm select
│   ├── ▶ PlaybackCard      ← Step Counter, ◀◀ ◀ ▶ ▶ ▶▶
│   └── ⚡ SpeedCard        ← Speed slider (ms)
├── DescriptionPanel（アルゴリズム説明、折りたたみ可能）
